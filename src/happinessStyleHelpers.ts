...
export function getGradientCreatorForLabel(label) {
  const { gradient: { start, end } } = getStyleForLabel(label)
...
export function getStyleForLabel(key): HappinessStyle {
  ...
  return {
    label: 'Workplace',
    tab: { text: '', font: '', gradient: { start: '#162438', end: '#162438'} },
  ...
  labels: {
    colValue: {
      ...
      display: function (context) {
        const { dataset: { label = '', data = [] } = {}, datasetIndex, dataIndex } = context
      ...
      formatter: function (value, context) {
        const { dataset: { label = '' } = {}, datasetIndex } = context
    ...
    value: {
      ...
      color: function (context) {
        ...
        console.log(
          ...
          getStyleForLabel(datasetIndex > 0 ? 'Workplace' : value.x).dataFont.color
        )
